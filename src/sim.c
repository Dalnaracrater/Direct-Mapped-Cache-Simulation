 ...
 * Ex. Format:
 *  ----------------------------------------------------------
 * | Tag: 18 bits ' ' Index: 12 bits ' ' Byte Select: 2 bits |
 *  ----------------------------------------------------------
 * 2 space character are added.
 *
 * Ex. Result:
 ...
 *  4. Read a line from the trace file
...
Cache createCache(int cache_size, int block_size, int write_policy){
    ...
    if(write_policy != WRITETHROUGH && write_policy != WRITEBACK){
